...
const [showDot, setShowDot] = React.useState(true);
  ...
  const organizationRows = useSelector((state) => state.organizationsReducer);
  ...
  const org = useSelector((state) => (currentId ? state.organizationsReducer.find((message) => message._id === currentId) : null));
